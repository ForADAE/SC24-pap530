...
__all__ = [
    'RNNBase', 'RNN', 'LSTM', 'GRU', 'RNNCellBase', 'RNNCell', 'LSTMCell',
    'GRUCell'
]
...
def log_memory_usage(message):
    ...
    print(
        f"[MEM_LOG] {memory_diff:>20} {current_memory_usage:>20} [LSTM] {message}"
    )


def estimate_lstm_memory_usage(input_size,
                               hidden_size,
                               batch_size,
                               seq_len,
                               dtype=torch.float32):
    num_params = 4 * (input_size * hidden_size + hidden_size**2 + hidden_size)
    intermediate_results_per_step = 4 * batch_size * hidden_size
    total_intermediate_results = seq_len * intermediate_results_per_step
    gradients = num_params
    adam_memory = 2 * num_params

    total_memory = 4 * (num_params + total_intermediate_results + gradients +
                        adam_memory)
...
def _apply_permutation(tensor: Tensor,
                       permutation: Tensor,
                       dim: int = 1) -> Tensor:
...
class RNNBase(Module):
    ...
    def __init__(self,
        ...
        self._flat_weight_refs: List[Optional[
            weakref.ReferenceType[Parameter]]] = []
            ...
            for direction in range(num_directions):
                ...
                w_ih = Parameter(
                    torch.empty((gate_size, layer_input_size),
                                **factory_kwargs))
                w_hh = Parameter(
                    torch.empty((gate_size, real_hidden_size),
                                **factory_kwargs))
                ...
                if self.proj_size == 0:
                    ...
                    w_hr = Parameter(
                        torch.empty((proj_size, hidden_size),
                                    **factory_kwargs))
                    if bias:
    ...
    def _init_flat_weights(self):
        ...
        self._flat_weight_refs = [
            weakref.ref(w) if w is not None else None
            ...
            with torch.no_grad():
                if torch._use_cudnn_rnn_flatten_weight():
                    ...
                    torch._cudnn_rnn_flatten_weight(
                        self._flat_weights, num_weights, self.input_size,
    ...
    def check_input(self, input: Tensor,
        ...
        if not torch.jit.is_scripting():
            if input.dtype != self._flat_weights[
                    0].dtype and not torch._C._is_any_autocast_enabled():
                raise ValueError(
...
class RNN(RNNBase):
    ...
    def forward(self,
                ...
                hx: Optional[Tensor] = None) -> Tuple[PackedSequence, Tensor]:
        pass
    ...
    def forward(self, input, hx=None):  # noqa: F811
        ...
        if batch_sizes is None:
            if self.mode == 'RNN_TANH':
                result = _VF.rnn_tanh(input, hx, self._flat_weights, self.bias,
                                      self.num_layers, self.dropout,
                                      self.training, self.bidirectional,
                                      self.batch_first)
            else:
                result = _VF.rnn_relu(input, hx, self._flat_weights, self.bias,
                                      self.num_layers, self.dropout,
                                      self.training, self.bidirectional,
                                      self.batch_first)
        else:
            if self.mode == 'RNN_TANH':
                result = _VF.rnn_tanh(input, batch_sizes, hx,
                                      self._flat_weights, self.bias,
                                      self.num_layers, self.dropout,
                                      self.training, self.bidirectional)
            else:
                result = _VF.rnn_relu(input, batch_sizes, hx,
                                      self._flat_weights, self.bias,
                                      self.num_layers, self.dropout,
                                      self.training, self.bidirectional)
...
class LSTM(RNNBase):
    ...
    def check_forward_args(
        ...
        self.check_hidden_size(
            hidden[0], self.get_expected_hidden_size(input, batch_sizes),
            'Expected hidden[0] size {}, got {}')
        self.check_hidden_size(hidden[1],
    ...
    def forward(self, input, hx=None):  # noqa: F811
        ...
        self._update_flat_weights()
        log_memory_usage("self._update_flat_weights()")

        orig_input = input
        log_memory_usage("orig_input = input")
        batch_sizes = None
        do_permute = False
        num_directions = 2 if self.bidirectional else 1
        ...
        if isinstance(orig_input, PackedSequence):
            ...
            log_memory_usage(
                "input, batch_sizes, sorted_indices, unsorted_indices = input")
            max_batch_size = batch_sizes[0]
            log_memory_usage("max_batch_size = batch_sizes[0]")
            if hx is None:
                ...
                hx = (h_zeros, c_zeros)
                log_memory_usage("hx = (h_zeros, c_zeros)")
            else:
                hx = self.permute_hidden(hx, sorted_indices)
                log_memory_usage(
                    "hx = self.permute_hidden(hx, sorted_indices)")
        ...
        if batch_sizes is None:
            ...
            result = _VF.lstm(input, batch_sizes, hx, self._flat_weights,
        ...
        output = result[0]
        log_memory_usage("output = result[0]")
        hidden = result[1:]
        log_memory_usage("hidden = result[1:]")
        if isinstance(orig_input, PackedSequence):
            output_packed = PackedSequence(output, batch_sizes, sorted_indices,
                                           unsorted_indices)
            log_memory_usage(
            ...
            if not is_batched:
                output = output.squeeze(batch_dim)
                log_memory_usage("output = output.squeeze(batch_dim)")
...
class RNNCellBase(Module):
    ...
    def __init__(self,
        ...
        self.weight_ih = Parameter(
            torch.empty((num_chunks * hidden_size, input_size),
                        **factory_kwargs))
        self.weight_hh = Parameter(
            torch.empty((num_chunks * hidden_size, hidden_size),
                        **factory_kwargs))
        if bias:
...
class LSTMCell(RNNCellBase):
    ...
    def forward(self,
        ...
        if hx is None:
            ...
            hx = (hx[0].unsqueeze(0),
                  hx[1].unsqueeze(0)) if not is_batched else hx
